...
void solve(){
    ll n,m,k; cin>>n; char ch ;cin>>ch;
    string s; cin>>s;
    int ans = 0;
    if(ch == 'g')cout<<ans<<endl;
    else{
        int fg = s.find("g"), lg = -1, fr = s.find(ch), lr = -1;
        for(int i=n-1;i>=0;i--){
            if(s[i] == 'g') break;
            if(s[i] == ch) lr = i;
        }
        ...
        int cnt = (n-lr)+fg;
        if(lr!=-1)ans = max(cnt,ans);

        int pr = -1;
        for(int i=0;i<n;i++){
            if(s[i] == ch && pr == -1) pr = i;
            else if(s[i] == 'g' && pr != -1) ans = max(ans,i-pr),pr = -1;
