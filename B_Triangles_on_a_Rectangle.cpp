...
void solve(){
    ...
    for(int i=0;i<4;i++){
        ...
        for(int j=0;j<k;j++){
            ...
            else if(i==1) b.push_back(x);
            else if(i == 2)c.push_back(x);
            else d.push_back(x);
        }
    }
    ll ans = 0;
    ll area1 = (a[a.size()-1]-a[0])*h;
    ll area2 = (b[b.size()-1]-b[0])*h;
    ll area3 = (c[c.size()-1]-c[0])*w;
    ll area4 = (d[d.size()-1]-d[0])*w;
    ans = max(area2,max(area3,area4));
    ans = max(ans,area1);
    cout<<ans<<endl;
